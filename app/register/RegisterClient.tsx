      ...
      <div className="w-full max-w-md glass rounded-2xl p-8 border border-white/10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input id="username" value={username} onChange={e=>setUsername(e.target.value)} className="w-full bg-nexa-surface border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-nexa-violet outline-none" placeholder="Legend123" required />
          </div>
          <div>
            <label htmlFor="email" className="block text-sm text-text-secondary mb-1">Email</label>
            <input id="email" type="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-nexa-surface border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-nexa-violet outline-none" placeholder="you@example.com" required />
          </div>
          <div>
            <label htmlFor="password" className="block text-sm text-text-secondary mb-1">Password</label>
            <input id="password" type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-nexa-surface border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-nexa-violet outline-none" placeholder="••••••••" required />
          ...
          <div>
            ...
            <input id="confirm" type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} className="w-full bg-nexa-surface border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-nexa-violet outline-none" placeholder="••••••••" required />
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-nexa-violet hover:bg-violet-600 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition">{loading ? 'Creating...' : 'CREATE ACCOUNT'}</button>
