...
export default function InstantPlaySection({ game }: { game: Game }) {
  ...
  return (
    <div
      ref={containerRef}
      className={[
        'glass overflow-hidden border border-nexa-violet/25 bg-nexa-navy',
        'shadow-[0_0_40px_var(--nexa-violet-shadow)]',
        isFullscreen
          ? 'fixed inset-0 z-[9999] w-screen h-screen rounded-none flex flex-col'
          : 'rounded-3xl'
      ].join(' ')}
    >
      {/* Game toolbar */}
      <div
        className={[
          ...
          'bg-nexa-surface/95 border-b border-[color:var(--white-05)]',
          isFullscreen ? 'shrink-0' : ''
        ].join(' ')}
      >
        <div className="flex items-center gap-3 min-w-0">
          <div
            className={`w-8 h-8 shrink-0 rounded-lg flex items-center justify-center ${game.gradient} font-black text-[color:var(--text-primary)] text-sm`}
          >
            {game.initials}
          </div>

          <div className="min-w-0">
            <h3 className="text-[color:var(--text-primary)] font-bold text-sm truncate">
              {game.name}
            </h3>

            <p className="text-[10px] text-nexa-emerald">
              LIVE • {game.width}x{game.height} • {providerName}
            </p>
          </div>
        </div>
